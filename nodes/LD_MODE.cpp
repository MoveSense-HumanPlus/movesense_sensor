...
public:
  ...
  sensor_msgs::Image img_l_, img_d_, img_depth_;
  image_transport::CameraPublisher image_pub_l,image_pub_d,image_pub_depth;
      ...
      img_depth(NULL)
  {
    ...
    image_pub_d = it.advertiseCamera("disp/image_raw", 2);
    image_pub_depth = it.advertiseCamera("depth/image_raw", 3);
    ...
    // init
    if (0 == camera_mode.compare(std::string("CAM_STEREO_752X480_LD_30FPS")))
    {
        width = 752;
        height = 480;
        len = width*height*2;
    ...
    while (node_.ok())
    {
      ...
      if(len>0)
      {
          ...
          for(int i = 0 ; i < height; i++)
          {
              ...
              memcpy(img_d+width*i,	img_data+(2*i+1)*width,	width);
          }
          ...
          for(int i = 0 ; i < height; i++)
          {
              ...
              {
                  if (img_d[i*width + j] >128) img_depth[i*width + j]  = 4.0 * c_f * c_b / (img_d[i*width + j] - 128);
          ...
          fillImage(img_depth_, "mono16", height, width, width, img_depth);
          ...
          image_pub_depth.publish(img_d_,*ci_d);
      }
...
int main(int argc, char **argv)
{
  ...
  {
    ROS_INFO("The camera default launch mode is : 'CAM_STEREO_752X480_LD_30FPS'.\n"
